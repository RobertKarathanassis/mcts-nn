...
# ── project imports (match your folder tree) ────────────────────────────
from data import board, move_encoder as enc                 # data/board.py, data/move_encoder.py
from mcts.mcts import MCTS                                  # mcts/mcts.py
from network.network import JohnNet                          # network/network.py
...
# ── hyper-params ────────────────────────────────────────────────────────
NUM_SIMS    = 30
TEMP_CUTOFF = 30           # ply < 30 ⇒ τ=1, else τ=0
DIRICH_A    = 0.3
DIRICH_EPS  = 0.25
...
def play_game(worker_id: int, game_id: int, device: torch.device) -> None:
    ...
    z = np.array([z_final if t == turns[0] else -z_final for t in turns], dtype=np.float32)
